...
'        ' No DB match — show NHTSA data and run analysis
'        If dbFields Is Nothing OrElse dbFields.Count = 0 Then
'            PopulateNhtsaGridFull(nhtsaFields)
'            lblStatus.Text = "No matching partial VIN found in database."
'            lblStatus.ForeColor = Color.DimGray
'            AnalyseAndSuggest(vin, nhtsaFields)
'            Return
'        End If
...
'            lblStatus.Text = $"⚠ {discrepantFields.Count} discrepancy(ies) found between NHTSA and database."
'            lblStatus.ForeColor = Color.Crimson
'        Else
'            lblStatus.Text = "✓ No discrepancies found!"
...
'    '  ANALYSE AND SUGGEST
'    '
'    '  Primary signal: did the VDS decode cleanly?
'    '  If NHTSA returned Make, Model AND Body Class, the VDS (positions 4-8)
'    '  is fine — any remaining issue is in position 10 (year) or 11 (plant).
'    '  If those fields are missing or sparse, the VDS is suspect.
'    ' -----------------------------------------------------------------------
'    'Private Sub AnalyseAndSuggest(vin As String, nhtsaFields As Dictionary(Of String, String))

'    '    ' The VDS decoded cleanly if NHTSA returned all three of these.
'    '    ' They are entirely driven by positions 1-8, so their presence means
'    '    ' the VDS is valid regardless of what error codes say.
'    '    Dim vdsDecodedCleanly As Boolean =
'    '        nhtsaFields.ContainsKey("Make") AndAlso
'    '        nhtsaFields.ContainsKey("Model") AndAlso
'    '        nhtsaFields.ContainsKey("Body Class")

'    '    If vdsDecodedCleanly Then
'    '        ' VDS is fine — show year verification controls so the user can
'    '        ' check whether position 10 matches their known model year.
'    '        Label5.Visible = True
'    '        txtYear.Visible = True
'    '        btnCheckYear.Visible = True
'    '    Else
'    '        ' VDS is suspect — try to suggest a correction using whatever
'    '        ' NHTSA gives us: a direct suggested VIN, or possible values.
'    '        Dim nhtsaSuggestedVin As String = ""
'    '        If nhtsaFields.ContainsKey("Suggested VIN") Then
'    '            nhtsaSuggestedVin = nhtsaFields("Suggested VIN").Trim().ToUpper()
'    '        End If

'    '        If Not String.IsNullOrWhiteSpace(nhtsaSuggestedVin) AndAlso nhtsaSuggestedVin.Length = 17 Then
'    '            lblStatus.Text = $"⚠ VDS issue detected. Suggested corrected VIN: {nhtsaSuggestedVin}"
'    '            lblStatus.ForeColor = Color.DarkOrange
'    '            btnCheckCorrectedVin.Tag = nhtsaSuggestedVin
'    '            btnCheckCorrectedVin.Visible = True

'    '        ElseIf nhtsaFields.ContainsKey("Possible Values") Then
'    '            Dim possibleValues As String = nhtsaFields("Possible Values").Trim()
'    '            Dim suggestedVin As String = ApplyPossibleValues(vin, possibleValues)

'    '            If suggestedVin <> vin Then
'    '                lblStatus.Text = $"⚠ VDS issue detected. Possible values: {possibleValues}  →  Suggested VIN: {suggestedVin}"
'    '                lblStatus.ForeColor = Color.DarkOrange
'    '                btnCheckCorrectedVin.Tag = suggestedVin
'    '                btnCheckCorrectedVin.Visible = True
'    '            Else
'    '                lblStatus.Text = $"⚠ VDS issue detected. Possible values: {possibleValues} (could not auto-build a corrected VIN)."
'    '                lblStatus.ForeColor = Color.DarkOrange
'    '            End If

'    '        Else
'    '            lblStatus.Text = "⚠ VDS (digits 4-8) appears suspect but NHTSA did not return a suggested correction."
'    '            lblStatus.ForeColor = Color.DarkOrange
'    '        End If
'    '    End If
'    'End Sub
...
'        If nhtsaFields.ContainsKey("Error Code") Then
'            For Each code In nhtsaFields("Error Code").Split(","c)
'                errorCodes.Add(code.Trim())
'            Next
'        End If

'        ' VDS decoded cleanly if:
'        '   (1) Make is present — WMI was recognized
'        '   (2) Error code 14 is absent — no VDS pattern ambiguity
'        ' Body Class is intentionally excluded because it depends on model year
'        ' and will be missing whenever position 10 is wrong, even with a valid VDS.
'        Dim vdsDecodedCleanly As Boolean =
'        nhtsaFields.ContainsKey("Make") AndAlso
'        Not errorCodes.Contains("14")
...
'        Dim expectedChar As String = yearToVinChar(modelYear)
'        Dim actualChar As String = _lastInputVin(9).ToString()
'        Dim plantChar As String = _lastInputVin(10).ToString()

'        If expectedChar = actualChar Then
'            lblStatus.Text = $"✓ Year digit '{actualChar}' at position 10 is correct for {modelYear}. " &
'                             $"⚠ Position 11 (plant code '{plantChar}') may be incorrect."
'            lblStatus.ForeColor = Color.DarkOrange
'        Else
'            lblStatus.Text = $"✗ Position 10 mismatch: found '{actualChar}', expected '{expectedChar}' for {modelYear}. " &
'                             $"Position 11 (plant code '{plantChar}') may also be incorrect."
'            lblStatus.ForeColor = Color.Crimson
'        End If
'    End Sub
